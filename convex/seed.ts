...
export const seed = mutation({
  ...
  handler: async (ctx) => {
    ...
    const hub1 = await ctx.db.insert("hub_collections", {
      ...
      zelle_contact: "payments@tolobana.org",
      payment_url: "",
      is_live: true,
      member_portal_audience: "all_members",
      verification_sla_business_days: 3,
      created_at: Date.now(),
      updated_at: Date.now(),
    });
    ...
    await ctx.db.insert("hub_contributions", {
      ...
      amount: 150,
      currency: "USD",
      reference_code: "REF-1501",
      status: "pending_verification",
      confirmation_number: "Z123456",
      donor_claimed_sent_at: Date.now(),
    ...
    await ctx.db.insert("hub_contributions", {
      ...
      payment_verified_at: Date.now() - 3600000,
      confirmation_number: "Z654321",
      donor_claimed_sent_at: Date.now() - 7200000,
